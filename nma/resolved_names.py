...
class rxnorm:
    ...
    def get_related_info(self, drug_term):
        '''
        ...
        if xml.content is not None:
            rxcui_list = self._etxract_unique_rxcuis(xml)
               
        for rxcui in rxcui_list:
            if rxcui is None:
                continue
            print (f'Processing {rxcui} from the list {rxcui_list}')
            IN = 'IN+SCDC+SCD'
            MIN = 'MIN+SCDC+SCD' #'multi-ingredient'
            
            if '/' in self.drug_name or 'and' in self.drug_name or ',' in self.drug_name: #its multi-ingredient
    ...
    def clean_terms(self, list_of_drug_terms):
        all_drug_related_terms = {}
        
        for concept in list_of_drug_terms:
            for term in concept.split(','):
                main_content, components, drugs = self.get_related_info(term)
                if components or drugs:
                    all_drug_related_terms.update({
                            term: [', '.join(main_content).strip(', '), ', '.join(components).strip(', '), ', '.join(drugs).strip(', ')]
                            })
        all_drug_related_terms = pd.DataFrame.from_dict(all_drug_related_terms, orient='index', 
                                                        columns=['Resolved', 'SCDC', 
                                                                 'SCD']).reset_index().rename(columns={'index':'Terms'})
...
class trial_register_handler(rxnorm):
    ...
    def resolve_terms(self):
        terms_df = self.clean_terms(self.list_of_drug_terms)
        return terms_df
...
if __name__=='__main__':  
    ...
    all_terms = rx.clean_terms(all_names)
    all_terms.to_csv('/home/ja18581/nma_project/data/rxnav_terms.csv', index = False)
    all_terms.to_excel('/home/ja18581/nma_project/data/rxnav_terms.xlsx', index = False)
